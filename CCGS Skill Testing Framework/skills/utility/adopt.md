...
after explicit user approval via `AskQuestion`.
...
- [ ] Contains "May I write" or `AskQuestion` language before writing the adoption plan
...
8. Uses `AskQuestion` to ask about writing the plan; user selects write
...
- [ ] `AskQuestion` is used before writing the adoption plan
...
6. `AskQuestion` asks to write the plan; writes after approval
...
- [ ] `AskQuestion` is used before writing
...
6. `AskQuestion` to write plan; writes after approval
...
**Assertions:**
- [ ] `AskQuestion` is used (not a plain text message) when no artifacts are found
...
- [ ] Uses `AskQuestion` before writing the adoption plan file
