...
against sprint stories, and uses `AskQuestion` to batch-verify manual smoke
...
5. Uses `AskQuestion` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `AskQuestion` is used for manual smoke check batches
...
### Case 3: Manual Confirmation — AskQuestion used, PASS WITH WARNINGS
...
3. `AskQuestion` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
**Assertions:**
- [ ] `AskQuestion` is used for manual smoke check batches (not inline text prompts)
...
- [ ] Uses `AskQuestion` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific AskQuestion batches and a
