...
and world-builder in parallel). Uses `AskQuestion` at each phase transition to
...
- [ ] `AskQuestion` is used at phase transitions before proceeding
...
2. `AskQuestion` presents the narrative brief; user approves before Phase 2 begins
...
4. `AskQuestion` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `AskQuestion` presents level narrative plan; user approves before Phase 4 begins
...
8. `AskQuestion` presents review results; user approves before Phase 5 begins
...
- [ ] `AskQuestion` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer Task calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `AskQuestion` is approved
...
6. `AskQuestion` presents options:
...
- [ ] `AskQuestion` presents at least 3 options including "stop and resolve first"
...
- [ ] No `AskQuestion` is used — output is direct guidance
...
5. `AskQuestion` presents options:
...
- [ ] `AskQuestion` offers the option to fix now vs. flag and proceed
...
- [ ] `AskQuestion` offers at least one option to resolve the missing prerequisite
...
- [ ] `AskQuestion` is used after every phase output before the next phase launches
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `AskQuestion`
