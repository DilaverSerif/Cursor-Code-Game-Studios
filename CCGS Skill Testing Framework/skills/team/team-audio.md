...
`AskQuestion` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] `AskQuestion` is used at step transitions before proceeding
...
3. `AskQuestion` presents audio direction; user approves before Step 2 begins
...
5. `AskQuestion` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
7. `AskQuestion` presents technical plan; user approves before Step 4 begins
...
- [ ] `AskQuestion` appears after Step 1 output and before Step 2 launch
...
- [ ] gameplay-programmer is not launched until Step 3 `AskQuestion` is approved
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `AskQuestion`
4. `AskQuestion` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `AskQuestion` surfaces the gap before Step 3 is launched
...
- [ ] `AskQuestion` is used after every step output before the next step launches
