...
**If no argument is provided**, do not fail — ask instead. Use AskQuestion:
...
> "Player journey map not yet created. Template available at `docs/studio/templates/player-journey.md`. Run `/ux-design` Phase 2b or create it manually to establish player context for this screen."
...
2. **Questions**: Ask what is needed to draft this section. Use AskQuestion
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use AskQuestion to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use AskQuestion:
...
7. **Write**: Use AskQuestion: "May I write the [section name] section to `[filepath]`?"
...
- Use AskQuestion to capture the choice:
...
- Use AskQuestion: "Want an ASCII wireframe as part of this spec?"
...
Use AskQuestion to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use AskQuestion:
...
- prompt: "Run `/ux-review [filename]` now, or do something else first?"
- options:
  - id: a, label: "- \"Run"
  - id: b, label: "ux-review` now — validate this spec\""
