...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and Use the AskQuestion tool with:
- prompt: "The next unspecced item is **[name]**. Generate specs for it?"
- options:
  - id: a, label: "[A] Yes — spec [name]"
  - id: b, label: "[B] Pick a different item"
  - id: c, label: "[C] Stop here"
...
Present the full proposed inventory to the user in conversation. Then Use the AskQuestion tool with:
...
- options:
  - id: a, label: "Looks good — save this inventory — `[B] Add items I'll describe`"
  - id: c, label: "Remove items that don't apply — `[D] Both add and remove — let me edit`"
Wait for the answer before continuing.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use AskQuestion: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, Use the AskQuestion tool with:
- prompt: "No profile found for **[name]**. Describe it briefly — a sentence or two is enough."
- options:
  - id: a, label: "[A] Describe it now"
  - id: b, label: "[B] Skip this entity"
  - id: c, label: "[C] Stop here"
Wait for the answer before continuing.
...
Present the full identified list to the user. Use the AskQuestion tool with:
- prompt: "I identified [N] assets across [N] categories for **[target]**. Review before speccing:"
- options:
  - id: a, label: "[A] Proceed — spec all of these"
  - id: b, label: "[B] Remove some assets"
  - id: c, label: "[C] Add assets I didn't catch"
  - id: d, label: "[D] Adjust categories"
Wait for the answer before continuing.
...
After presenting all specs, Use the AskQuestion tool with:
- prompt: "Asset specs for **[target]** — [N] assets. Review complete?"
- options:
  - id: a, label: "[A] Approve all — write to file"
  - id: b, label: "[B] Revise a specific asset"
  - id: c, label: "[C] Regenerate with different direction"
Wait for the answer before continuing.
...
- options:
  - id: a, label: "Spec another system — /asset-spec system:[next-system] — `[B] Spec a level — /asset-spec level:[level-name]`"
  - id: c, label: "Spec a character — /asset-spec character:[character-name] — `[D] Run /asset-audit — validate delivered assets against specs`"
  - id: e, label: "Stop here"
Wait for the answer before continuing.
