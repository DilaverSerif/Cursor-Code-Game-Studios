...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use AskQuestion** at key decision points throughout brainstorming:
   ...
   AskQuestion to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an AskQuestion with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use AskQuestion for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab AskQuestion with these exact tab labels:
...
Present all three. Then use AskQuestion to capture the selection.
...
Ask these as AskQuestion calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, Use the AskQuestion tool with:
- prompt: "Do these pillars feel right for your game?"
- options:
  - id: a, label: "[A] Lock these in"
  - id: b, label: "[B] Rename or reframe one"
  - id: c, label: "[C] Swap a pillar out"
  - id: d, label: "[D] Something else"
Wait for the answer before continuing.

If the user selects B, C, or D, make the revision, then Use the AskQuestion tool with:
- prompt: "Pillars updated. Ready to lock these in?"
- options:
  - id: a, label: "[A] Lock these in"
  - id: b, label: "[B] Revise another pillar"
  - id: c, label: "[C] Something else"
Wait for the answer before continuing.
...
Collect both verdicts, then present them together using a two-tab AskQuestion:
...
- **Target platform**: Use the AskQuestion tool with:
- prompt: "What platforms are you targeting for this game?"
- options:
  - id: a, label: "PC (Steam"
  - id: b, label: "Epic)"
  - id: c, label: "Mobile (iOS"
  - id: d, label: "Android)"
  - id: e, label: "Console"
  - id: f, label: "Web"
  - id: g, label: "Browser"
  - id: h, label: "Multiple platforms"
Wait for the answer before continuing.

- **Engine experience**: Use the AskQuestion tool with:
- prompt: "Do you already have an engine you work in?"
- options:
  - id: a, label: "Godot"
  - id: b, label: "Unity"
  - id: c, label: "Unreal Engine 5"
  - id: d, label: "No preference — help me decide"
Wait for the answer before continuing.
...
5. Use the AskQuestion tool with:
- prompt: "Game concept is ready. May I write it to `design/gdd/game-concept.md`?"
- options:
  - id: a, label: "[A] Yes — write it"
  - id: b, label: "[B] Not yet — revise a section first"
Wait for the answer before continuing.

If [B]: ask which section to revise using AskQuestion with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then Use the AskQuestion tool with:
- prompt: "Ready to write the updated concept document?"
- options:
  - id: a, label: "[A] Yes — write it"
  - id: b, label: "[B] Revise another section"
Wait for the answer before continuing.
...
> to `design/gdd/game-concept.md`. Open a fresh Cursor session to continue
