...
Before planning, list `production/sprints/` if that directory exists.
...
- If the file doesn't exist and this is a `new` sprint: Use the AskQuestion tool with:
- prompt: "No review mode is set. Which review depth would you like for this sprint?"
- options:
  - id: a, label: "full — spawn all director and lead gates — `[B] lean — skip non-phase-gate director reviews (recommended for most sprints)`"
  - id: c, label: "solo — skip all gate spawning — After selection: write `production/review-mode.txt` with the chosen mode. Say: \"Review mode set to [mode] and saved to production/review-mode.txt.\""
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use AskQuestion to gather changes.
...
If CONCERNS, Use the AskQuestion tool with:
- prompt: "Producer flagged concerns with this sprint plan. How do you want to proceed?"
- options:
  - id: a, label: "Proceed as planned — I accept the risk — `[B] Adjust scope — defer some Should Have stories`"
  - id: c, label: "Extend the sprint timeline"
Wait for the answer before continuing.
