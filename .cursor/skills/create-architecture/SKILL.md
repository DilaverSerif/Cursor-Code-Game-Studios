...
- options:
  - id: a, label: "Proceed — flag HIGH RISK domains throughout the output — `[B] Let me check the engine reference first — pause here`"
  - id: c, label: "Show me which domains are HIGH RISK and why"
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then Use the AskQuestion tool with:
- prompt: "All sections approved. May I write the master architecture document?"
- options:
  - id: a, label: "Yes — write to `docs/architecture/architecture.md` now"
  - id: b, label: "Show me the full draft inline first, then ask again — [C] Not yet — I have more changes to discuss"
Wait for the answer before continuing.
...
Use the AskQuestion tool with:
- prompt: "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
- options:
  - id: a, label: "Accept — proceed to handoff"
  - id: b, label: "Revise flagged items first"
  - id: c, label: "Discuss specific concerns"
Wait for the answer before continuing.
...
Show the proposed Document Status block inline, then Use the AskQuestion tool with:
- prompt: "May I update the Document Status section with the sign-off results?"
- options:
  - id: a, label: "Yes — apply to `docs/architecture/architecture.md`"
  - id: b, label: "Not yet — I want to revisit the concerns first"
Wait for the answer before continuing.
...
5. **Use AskQuestion for write approvals** — plain text "May I?" is not
