...
If the check passes, Use the AskQuestion tool with:
- prompt: "Ready to create hotfix branch 'hotfix/[short-name]' from [base-ref]?"
- options:
  - id: a, label: "Yes — create branch — `[B] Use a different base ref — I'll specify it`"
  - id: c, label: "Skip — I'll create the branch myself"
...
- options:
  - id: a, label: "Run /smoke-check to verify the fix — `[B] Run /patch-notes to document this hotfix`"
