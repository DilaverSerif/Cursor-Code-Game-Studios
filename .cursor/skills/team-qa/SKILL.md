...
Present the qa-lead's full strategy to the user, then use AskQuestion:
...
Use AskQuestion per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use AskQuestion for each:
...
After each FAIL result: use AskQuestion to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
