...
- `/regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, Use the AskQuestion tool with:
- prompt: "No subcommand specified. Which mode do you want to run?"
- options:
  - id: a, label: "update — scan new bug fixes this sprint and add missing regression tests — `[B] audit — full audit of all GDD critical paths vs. existing test coverage`"
  - id: c, label: "report — read-only status report (no writes)"
