...
a **confirm/adjust** prompt using AskQuestion — not open-ended questions.
...
decisions remain, present each one as a separate AskQuestion with the proposed
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`docs/studio/director-gates.md`):
...
5. **Write approval** — Use AskQuestion:
...
After the ADR is written (and registry optionally updated), close with AskQuestion.
...
> To validate ADR coverage against your GDDs, open a **fresh Cursor session**
