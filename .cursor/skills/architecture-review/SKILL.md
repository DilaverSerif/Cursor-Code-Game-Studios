...
Then Use the AskQuestion tool with:
- prompt: "I found [N] GDD revision flag(s). May I update the systems index?"
- options:
  - id: a, label: "Yes — apply all [N] updates to the systems index now"
  - id: b, label: "Show me the full diff first, then ask again — [C] No — leave the systems index unchanged for now"
...
If [B]: display the complete proposed systems-index section, then re-ask with AskQuestion.
...
- prompt: "Review complete. What would you like to write?"
- options:
  - id: a, label: "Write all three files (review report + traceability index + TR registry) — [B] Write review report only — `docs/architecture/architecture-review-[date].md`"
  - id: c, label: "Don't write anything yet — I need to review the findings first"
Wait for the answer before continuing.

### RTM Output (rtm mode only)

For `rtm` mode, Use the AskQuestion tool with:
- prompt: "May I write the full Requirements Traceability Matrix?"
- options:
  - id: a, label: "Yes — write to `docs/architecture/requirements-traceability.md`"
  - id: b, label: "Not yet — show me the full RTM data first, then ask again"
Wait for the answer before continuing.
...
Then close with AskQuestion tailored to the pre-gate checklist state:
...
3. **Offer options** via AskQuestion with three choices:
