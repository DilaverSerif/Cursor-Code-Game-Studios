...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and Use the AskQuestion tool with:
- prompt: "The next system in your design order is **[system-name]** ([priority] | [layer]). Start designing it?"
- options:
  - id: a, label: "[A] Yes — design [system-name]"
  - id: b, label: "[B] Pick a different system"
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   AskQuestion for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use AskQuestion to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   AskQuestion. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use AskQuestion with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use the AskQuestion tool with:
- prompt: "Is this system something the player engages with directly, or infrastructure they experience indirectly?"
- options:
  - id: a, label: "[A] Direct — player actively uses or feels this system"
  - id: b, label: "[B] Indirect — player experiences the effects, not the system"
  - id: c, label: "[C] Both — has a direct interaction layer and infrastructure beneath it"
Wait for the answer before continuing.
...
- Surface any disagreements between agents to the user via AskQuestion
...
- Present the specialists' proposals to the user for review via AskQuestion
...
> **To validate this GDD, open a fresh Cursor session and run:**
...
- options:
  - id: a, label: "- \"Run"
  - id: b, label: "consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)\""
Wait for the answer before continuing.
...
- The main session presents the agent's output to the user via AskQuestion
- The user decides; the main session writes to file
...
2. **AskQuestion** at every decision point (Explain -> Capture pattern):
...
> open a fresh Cursor session and run `/design-system [system-name]` — it will
