...
If they differ, Use the AskQuestion tool with:
...
- options:
  - id: a, label: "Update story manifest version and implement with current rules (Recommended) — `[B] Implement with old rules — I accept the risk of non-compliance`"
  - id: c, label: "Stop here — I want to review the manifest diff first"
...
- options:
  - id: a, label: "Proceed anyway — I accept the dependency risk — `[B] Stop — I'll complete the dependency first`"
  - id: c, label: "The dependency is done but status wasn't updated — mark it Complete and continue — If [B]: set story status to **BLOCKED** in session state and stop. Do not spawn any programmer agent."
Wait for the answer before continuing.
...
5. **Engine preferences**: `docs/studio/technical-preferences.md` — read naming conventions and performance budgets
...
3. **Offer options** via AskQuestion with choices:
