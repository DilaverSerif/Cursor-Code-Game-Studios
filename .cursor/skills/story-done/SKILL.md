...
3. If multiple in-progress stories are found, use AskQuestion:
...
### Manual verification with confirmation (use AskQuestion)
...
Batch up to 4 manual verification questions into a single AskQuestion call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via AskQuestion
...
- `lean` → Use the AskQuestion tool with:
- prompt: "Code review is skipped in lean mode. Did you run `/code-review` on the implemented files?"
- options:
  - id: a, label: "- `Yes —"
  - id: b, label: "code-review passed or was approved with suggestions"
...
Present the verdict to the user. If CONCERNS, surface them via AskQuestion:
...
Use AskQuestion before writing anything:
...
- Use AskQuestion for the code review prompt and for batching manual
