...
- options:
  - id: a, label: "Yes — looks good, run the director review and write the manifest — `[B] Add rules — I have additional rules to include before writing`"
  - id: c, label: "Remove rules — some extracted rules should be dropped — `[D] Stop here — I need to review the ADRs first`"
...
- **CONCERNS** → surface via AskQuestion with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
...
- options:
  - id: a, label: "Yes — write to docs/architecture/control-manifest.md — `[B] Show me the full draft first, then ask again`"
  - id: c, label: "Not yet — I want to make more changes"
Wait for the answer before continuing.
