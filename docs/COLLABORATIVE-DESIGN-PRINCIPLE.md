...
## 🎛️ Structured Decision UI (AskQuestion)

Use the `AskQuestion` tool to present decisions as a **selectable UI** instead
...
(main session) calls `AskQuestion` at each decision point between phases:

```
[game-designer returns 3 combat approaches with analysis]

Orchestrator uses AskQuestion:
...
- **AskQuestion tool** — Integrated into 16 skills for structured option UI
