...
1. **Ask clarifying questions:**
   ...
   - *Use `AskQuestion` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `AskQuestion` to capture the decision*
...
Use the `AskQuestion` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `AskQuestion` call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via AskQuestion
