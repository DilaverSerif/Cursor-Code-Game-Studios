# Cursor Code Game Studios -- Complete Workflow Guide

> **How to go from zero to a shipped game using the Agent Architecture.**
>
> This guide walks you through every phase of game development using the
> 49-agent system, 73 slash commands, and 10 automated hooks. It assumes you
> have Cursor installed and are working from the project root.
>
...
Start a new Cursor Agent session. You should see output from the
`session-start.sh` hook:

```
=== Cursor Code Game Studios -- Session Context ===
Branch: main
Recent commits:
...
If you see this, hooks are working. If not, check `.cursor/hooks.json` to
...
Agents use the `AskQuestion` tool for structured option presentation.
...
| `validate-skill-change.sh` | Skill file written | Advises running `/skill-test` after `.cursor/skills/` changes |
